...
class CacheManager {
  ...
  async invalidateStudentCache(studentId) {
    try {
      const pattern = `student:${studentId}:*`;
      const keys = await redis.keys(pattern);
      if (keys.length > 0) {
        await redis.del(...keys);
        logger.info('Student cache invalidated', { studentId, keysDeleted: keys.length });
      }
    } catch (error) {
      logger.error('Cache invalidation error', { studentId, error: error.message });
    }
  }
